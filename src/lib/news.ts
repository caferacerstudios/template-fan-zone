// SFZ_DAILY_NEWS_V1
import { NEWS_SITE } from "./news-team.mjs";
...
export const NEWS_CATEGORIES = ["News", "Analysis", "Contract Strategy", "Roster", "Injuries", "Game Week", "Hard Knocks", "NFC West", "NFC East", "NFC North", "NFC South", "AFC West", "AFC East", "AFC North", "AFC South"] as const;
...
// Legacy authored stories belong to Seattle; never rename their facts or URLs.
export const authoredArticles = articles
  .map(article => ({ ...article, team: article.team ?? "seahawks", tags: [...new Set([...article.tags, article.team ?? "seahawks"])] }))
  .filter(article => article.team === NEWS_SITE.team);
const generatedArticles = validateGeneratedCollection(generatedNews).articles;
export const publishedArticles: NewsArticle[] = mergePublishedArticles(authoredArticles, generatedArticles);
