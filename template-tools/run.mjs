...
try {
  ...
  if (isBuild) await rm(path.join(root, 'dist'), { recursive: true, force: true });
  ...
  if (command === 'preview') {
    ...
    const count = await renderProject(root, target, team);
    console.log(`Rendered ${count} template files for ${team.name}: ${path.relative(root, target)}`);
  }
  console.log(`Theme: ${team.theme.key}. Team IDs, stadiums, facts, feeds, and existing image content still come from the original copy.`);
